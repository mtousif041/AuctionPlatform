...
const superAdminSlice = createSlice({
  name: "superAdmin",  //isko store me importe krana pdega 
  initialState: {
    loading: false,
    monthlyRevenue: [],
    totalAuctioneers: [],
...
////////////////////////////////////////////////////    
    ...
    successForAllUsers(state, action) {
      state.loading = false;
      state.totalAuctioneers = action.payload.auctioneersArray;// auctioneersArray and biddersArray dono superAdminController.js se aarhaye hai , jo humne ine vha se res.status me se bheje the 
...
export const getMonthlyRevenue = () => async (dispatch) => {
  ...
  try {
    const response = await axios.get(
      ...
      "http://192.168.43.226:5000/api/v1/superadmin/monthlyincome",
      { withCredentials: true }
    );
    dispatch(
      superAdminSlice.actions.successForMonthlyRevenue(
        response.data.totalMonthlyRevenue //totalMonthlyRevenue ye superAdminController se aarha hai  backend se 
      )
    );
  } catch (error) {
...
export const getAllUsers = () => async (dispatch) => {
  ...
  try {
    const response = await axios.get(
      ...
      "http://192.168.43.226:5000/api/v1/superadmin/users/getall",
      { withCredentials: true }
    );
...
export const getAllPaymentProofs = () => async (dispatch) => {
  ...
  try {
    const response = await axios.get(
      ...
      "http://192.168.43.226:5000/api/v1/superadmin/paymentproofs/getall",
      { withCredentials: true }
    );
    dispatch(
...
export const deletePaymentProof = (id) => async (dispatch) => {
  ...
  try {
    const response = await axios.delete(
      ...
      `http://192.168.43.226:5000/api/v1/superadmin/paymentproof/delete/${id}`,
      { withCredentials: true }
    );
...
export const getSinglePaymentProofDetail = (id) => async (dispatch) => {
  ...
  try {
    const response = await axios.get(
      ...
      `http://192.168.43.226:5000/api/v1/superadmin/paymentproof/${id}`,
      { withCredentials: true }
    );
    dispatch(
...
export const updatePaymentProof = (id, status, amount) => async (dispatch) => {
  ...
  try {
    const response = await axios.put(
      ...
      `http://192.168.43.226:5000/api/v1/superadmin/paymentproof/status/update/${id}`,
      { status, amount }, // ye as data jarha i think
