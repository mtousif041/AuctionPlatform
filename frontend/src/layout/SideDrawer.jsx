...
const SideDrawer = () => {
  const [show, setShow] = useState(false); // agr show true hogi choti screen pe navbaar show ho jayega warna hidden ho ayega
  const { isAuthenticated, user } = useSelector((state) => state.user); // ye user hmare pass store.js me se aaya hai
  //(state) => state.user knyuki userSlice me (state, action) hai
  //(state) => state.user humne yha pr => state.user me .user isliye liya hai knyunki store.js me humne user: userReducer, aise liya hai

  const dispatch = useDispatch();
  const handleLogout = () => {
    dispatch(logout()); // ye jo logout hai vo function wala logout hao jo humne userSlice me reducers ke niche bnaye hai
  };
  ...
  return (
    <>
      ...
      >
        ...
        {/* yaani agr show ki value true hai to false krdo false hai to true krdo */}
        <GiHamburgerMenu />
      </div>
      <div
      ...
      >
        {/* sm:w-[300px] means small screen, lg:left-0 means larg screen   */}
        <div className="relative">
          ...
          />
          {/* sm:hidden yaani small screen pr ye rhaga hidden  */}
        </div>
        ...
        <div>
          ...
          <p className="text-stone-500">&copy; Prime, LLC.</p>
