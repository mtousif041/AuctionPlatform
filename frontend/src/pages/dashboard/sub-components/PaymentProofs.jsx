...
export const Drawer = ({ setOpenDrawer, openDrawer }) => {
  ...
  const [amount, setAmount] = useState(singlePaymentProof.amount || ""); // agr singlePaymentProof.amount ye hmare pass exist krta ho to thik hai varna ise krdo empty
  const [status, setStatus] = useState(singlePaymentProof.status || "");

  const dispatch = useDispatch();
  const handlePaymentProofUpdate = () => {
    dispatch(updatePaymentProof(singlePaymentProof._id, status, amount)); // hum yha se ye theeno parameter bhej dhenge
  };
          ...
          <div className="w-full px-5  sm:max-w-[640px] sm:m-auto">  
            ...
            <form className="flex flex-col gap-5 my-5">
              <div className="flex flex-col gap-3">
                ...
                <input
                  type="text"
                  value={singlePaymentProof.userId || ""} // agr singlePaymentProof.userId ye hmare pass exist krta ho to thik hai varna ise krdo empty
                  disabled
              ...
              <div>
                <Link
                  ...
                  target="_blank" // for opening in new window
                >
                  Payment Proof (SS)
                </Link>
              </div>
