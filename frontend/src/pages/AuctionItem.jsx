...
const AuctionItem = () => {
  const { id } = useParams(); // url se id hum aise get krte hai
  // console.log(id);
  ...
  useEffect(() => {
    if (!isAuthenticated) {
      // yaani jab tak user authenticated nhai hoga vo us details page pr ja hi nhai payega , vo home page pr navigate ho jayega
      navigateTo("/");
    }
    if (id) {
      // yaani agr id exist krti hai to apko getAuctionDetail ko call kr dena hai
      dispatch(getAuctionDetail(id));
    }
  }, [isAuthenticated]);
          ...
          <div className="flex gap-4 flex-col lg:flex-row">
            <div className=" flex flex-col flex-1  gap-3">
              ...
              <p className="text-xl w-fit font-bold">
                ...
                <p className="text-sm mt-0 text-red-600">humne yha pr current bid isliye show nahi ki taki koi banda last second tak wait krke ek rupyaa bda kr bid na krde</p>
              <hr className="my-2 border-t-[1px] border-t-stone-700" />
              {auctionDetail.description && // jo discription hogi vo paragraph formaate me hogi aur usme jobi sentencense honge unko phele alag alg krenge split(". ") se array bna lenge , fir usme map methode run krenge
                        ...
                        >
                          <div className="flex items-center  gap-4">
                            <img
                              ...
                              alt={element.userName} // means ki agr profileImage nhai hui to  userName show krva dena hai
