...
import {
  ...
  FaFileInvoice,
  FaRedo,
} from "react-icons/fa"; // fa means font awsome
      ...
      <section className="w-full ml-0 m-0 h-fit px-5 pt-20 lg:pl-[320px] flex flex-col min-h-screen py-4 justify-center">
        ...
        <div className="flex flex-col gap-4 my-5">
          {steps.map((element, index) => {
            return (
              <div
                key={index} // hum jab bhi map method run krthe hai to humko ek key deni hoti hai , array ke ander jo index number hota hai vo hmesa unique rheta hai
                className="bg-white rounded-md p-2 lg:p-5 flex flex-col gap-2 group hover:bg-black transition-all duration-300"
              >
                <div
                  className="bg-black text-white p-3 text-xl rounded-full w-fit group-hover:bg-[#d6482b] transition-all duration-300"
                  //group-hover ka ye matlab hai ki jab aap sub se uper wala div yaani sabka parent div pr hover kroge to bhi is pr bhi hover ho jayega kyunki parent div me humne group likha hai , agr parent div group hta doge to ye kaam nhai krega
                >
                  {element.icon}
                </div>
                <h3
