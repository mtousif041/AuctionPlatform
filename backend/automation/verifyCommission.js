...
export const verifyCommissionCron = ()=>{
    cron.schedule("*/1 * * * *", async()=>{
        ...
        const approvedProofs = await PaymentProof.find({status: "Approved"}); // yaani jinka status approved ho un subko get kro 
                ...
                if(user){
                    if(user.unpaidCommission >= proof.amount){
                        updatedUserData = await User.findByIdAndUpdate(user._id, {
                            $inc:{
                                unpaidCommission: -proof.amount // yaani - se ye hoga ki ab ye increament ki jgha pr decrement krva dega 
                            },
                        },{new: true});
